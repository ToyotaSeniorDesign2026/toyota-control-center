...
> Fetch the complete documentation index at: https://fastmcp.wiki/llms.txt
> 
...
FastMCP uses an extensible environment system with a base `Environment` class that can be implemented by different environment providers. Currently, FastMCP supports the `UVEnvironment` for Python environment management using `uv`'s dependency resolver.
...
Network interface to bind when using HTTP transport:
...
Port number used for HTTP transport.
...
Working directory for the server process. Relative paths are resolved from the configuration file location.

**`args`** (`list[str]`)

Command-line arguments passed to the server after `--`, allowing them to be parsed by the server’s argument parser.
...
This configuration explicitly specifies the server entrypoint (`mcp`), making it clear which server instance or factory function to use.

Defaults used in this configuration:
