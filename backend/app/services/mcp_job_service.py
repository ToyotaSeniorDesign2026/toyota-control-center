...
"""MCP-backed job execution helpers for FastAPI run orchestration."""
...
async def _execute_direct_tool(execution_request: ExecutionRequest) -> dict[str, Any]:
...
async def _execute_agent(execution_request: ExecutionRequest) -> dict[str, Any]:
    ensure_control_center_importable()
...
async def _execute_job_async(execution_request: ExecutionRequest) -> dict[str, Any]:
    if execution_request.execution_mode == "direct_tool":
        execution = await _execute_direct_tool(execution_request)
    else:
        execution = await _execute_agent(execution_request)

    execution["job_spec"] = execution_request.job_spec
    return execution


def execute_job_via_mcp(execution_request: ExecutionRequest) -> dict[str, Any]:
    execution = asyncio.run(_execute_job_async(execution_request))
    metadata = {
